...
| Date | Change | Reason |
|---|---|---|
| 2026-04-21 | **H6 post-campaign polish — ui_agent schema fix, ChatAgent 45 s timeout, OTEL stderr routing.** Drove a live `tw2k serve` match end-to-end using Grok (`grok-4-1-fast-reasoning`) for both LLMAgent pilots while a human-simulating Cursor agent exercised every H6 surface (14 MCP tools via `TwkHttpClient`, `/api/economy/{prices,routes}`, copilot state / hints / safety / memory / whatif, mode spectrum, manual warp / trade / warp-blocked paths, OTEL bridge via isolation smoke); soak hit day 2 and captured a full Genesis → land → seed colonists → `build_citadel` L1 → `citadel_complete` sequence executed hands-off by P2. Full run + verdict in `docs/TEST_REPORT_H6.md`. Three findings logged; two landed as fixes here (the third, `turns_today` reset, was retracted on review — `tick_day()` in `runner.py:151` already resets all players including humans). **Fix 1 — hints schema drift:** `src/tw2k/copilot/ui_agent.py` was reading `sector.warps` / `port.buying` / `port.selling` / `ship.cargo_max` / `ship.hull` — none of which the current observation builder emits (`_sector_detail` uses `warps_out`, port dict uses `buys` / `sells` with structured `stock = {current,max,price,side}`, `_ship_dict` uses `holds` / `cargo_free` / `fighters`). Symptom: `GET /api/copilot/hints?player_id=P1` returned *"No visible warps — try scanning first."* even while `obs.sector.warps_out = [292, 406, 712, 874]`, and the trade hint never surfaced port prices. Full rewrite with `_warps()` / `_port_buys()` / `_port_sells()` / `_stock_qty()` / `_stock_price()` helpers that prefer current-schema keys but fall back to legacy keys so the `_fake_observation` fixture in `tests/test_copilot_phase_h2.py` keeps passing. Status-line now shows `cr · fighters · holds used/total · port code`. **Fix 2 — ChatAgent timeout:** `ChatAgent.timeout_s` default 25 s → 45 s in `src/tw2k/copilot/chat_agent.py`. 25 s was aggressive under Grok contention (P1 ChatAgent racing P2 + P3 LLMAgent on the same provider); the live test hit the `[copilot timed out after 25s] try rephrasing.` graceful-fallback path once. 45 s fits observed Grok p95 for long plan envelopes without feeling sluggish; failure path still returns a safe "try rephrasing" response with the chat thread + pending_plan untouched. **Fix 3 — OTEL console routing:** `TW2K_OTEL_CONSOLE=1` previously attached `ConsoleSpanExporter()` with default `out=sys.stdout`, which corrupts any JSON-emitting CLI (`tw2k human-sim … --json`) if the env var happens to be set globally — exposed by a failing `test_human_sim_cli_subcommand_returns_json`. `src/tw2k/copilot/otel.py::_ensure_tracer_provider` now builds `ConsoleSpanExporter(out=sys.stderr)` so traces still stream to the terminal but stdout stays parseable for downstream tooling. Module docstring updated. Tests re-run with and without `TW2K_OTEL_CONSOLE=1` — 362/362 green, ruff clean. No public API changes; no new tests needed (existing tests caught the CLI regression once the env var was set). | Run the match, find the bugs, fix them same-commit — routine post-ship polish that the live-play test campaign surfaced. |
| 2026-04-21 | **Phase H6 shipped — MCP server, OTEL tracing, economy dashboards (plus H6.2 / H6.5 design docs).** Three real features + two committed plans. **H6.1 MCP:** new `src/tw2k/mcp_server.py` exposes the copilot surface as a Model Context Protocol server (`tw2k mcp` CLI, 14 tools — `tw2k_list_humans` / `_get_observation` / `_get_copilot_state` / `_send_chat` / `_set_mode` / `_confirm_plan` / `_cancel_plan` / `_submit_action` / `_get_memory` / `_remember` / `_forget` / `_get_whatif` / `_get_safety` / `_get_hints`). Architecture: MCP server is a thin HTTP proxy over the running `tw2k serve` instance — no new match bootstrap, reuses every `/api/*` contract `/play` already uses, optional `TW2K_MCP_TOKEN` bearer auth. The `mcp` package is a **soft dep** (`pip install "tw2k-ai[mcp]"`); the underlying `TwkHttpClient` + `dispatch_tool` shell is always importable so unit tests run SDK-free. 6 new MCP tests (5 dispatch, 1 auth header, plus the registry-shape smoke) drive an in-process ASGI app via `httpx.ASGITransport`. Client config for Cursor / Claude Code lives in the module docstring. **H6.3 OTEL tracing:** new `src/tw2k/copilot/otel.py::CopilotOtelBridge` bolts onto `CopilotTracer` as an optional sink — one long-lived `copilot.session` span per player with every JSONL event (`chat_utterance`, `llm_call`, `action_dispatched`, `standing_order_block`, `safety_signal`, `escalation`, `mode_change`, `memory_update`, `task_state`) attached as span events, plus a discrete `copilot.action.<tool>` child span on every action dispatch for flamegraph-style flow visualisation. Env toggles: `TW2K_OTEL_ENDPOINT` (OTLP HTTP — Jaeger / Weave / Honeycomb), `TW2K_OTEL_SERVICE_NAME`, `TW2K_OTEL_CONSOLE` (stdout sink for debugging without a collector). Registry wiring in `CopilotRegistry.rebuild()` calls `build_bridge(player_id, attributes={tw2k.agent_name})` and hands the result to `CopilotTracer(otel_bridge=…)`; `CopilotRegistry.clear()` now calls `tracer.shutdown()` so session spans end cleanly on match restart. All OTEL imports are soft — `pip install "tw2k-ai[otel]"` is opt-in and the bridge returns `None` when the SDK is missing OR no endpoint is configured. 4 new tests drive an in-memory exporter to assert session span + events + per-action child spans fire end-to-end. **H6.4 Economy dashboards:** new `src/tw2k/copilot/dashboards.py::build_price_table` / `build_route_table` pure functions + `GET /api/economy/prices` + `GET /api/economy/routes` endpoints — server-computed live prices (respects fog-of-war: only `player.known_ports` entries are surfaced) + top-N trade-route ranking by credits-per-turn across all known port pairs (BFS round-trip hops, honours cargo holds, filters negative-profit pairs, skips unreachable pairs). Cockpit gets a new collapsible `<details class="economy-panel">` in the right column with a "Top trade routes" ordered list (click a route to plot course to its buy port) + a mini price heatmap (sector × commodity table, green cells = port sells cheap, amber cells = port buys at premium, stale rows dimmed after 3 in-game days). Panel re-fetches on every `refreshObservation()` tick. 6 new backend tests cover price-table shape, route ranking by profit_per_turn, empty state with a single known port, and the two endpoints end-to-end. 8 new UI smoke tests cover markup, CSS classes, `refreshEconomy` wiring, `/api/economy/*` references, and `plot_course` click-through. **H6.2 local-STT plan** committed as `docs/LOCAL_STT_PLAN.md` — recommends `faster-whisper` over whisper.cpp / Vosk / WebGPU, sketches `/ws/stt` WebSocket, `src/tw2k/stt/` module layout, `initial_prompt` vocabulary biasing from the observation + memory, settings radio in `/play`, optional `tw2k-ai[stt]` extra. Implementation deferred. **H6.5 multi-human plan** committed as `docs/MULTI_HUMAN_PLAN.md` — slot picker, soft-lock claim/release/kick, team chat sub-channel, per-slot turn timer with auto-wait, `team_broadcast` tool with rate limit + safety gating, `BroadcastChannel` collision detection for same-machine multi-browser mic contention. WebRTC voice channels deferred further. Implementation deferred. New optional-deps in `pyproject.toml`: `mcp` (for H6.1) + `otel` (for H6.3). 22 new H6 tests (14 + 8); ruff + pytest green at 362 total. | User greenlit 1/3/4 as ship-now + 2/5 as plan-only. H6 delivers three fresh operator-grade surfaces (programmatic control via MCP, observability via OTEL, in-game economy UX) and locks in the design for the two bigger-investment features (local voice, multiplayer humans) so the next session can execute without re-scoping. |
...
| 2026-04-20 | **H5.5 post-ship polish — manual-cockpit fix, autopilot unblocker, cockpit navigation.** Live-match play session turned up three real bugs that shipped inside H5 because they required end-to-end browser + LLM exercise to surface. Fix 1 (manual cockpit — **blocker**): `web/play.js` `ACTION_SPECS` was emitting `args.to` for `warp` / `probe` / `hail`, but the engine (`_handle_warp` / `_handle_probe` / `_handle_hail` in `src/tw2k/engine/runner.py`) only reads `args.target` — every adjacent-sector click in `/play` was being rejected with *"warp requires 'target' sector id"* and the observation's `action_hint` accumulated *"REPEATED FAILURES … unknown rejected xN"*. All three `buildArgs` lambdas now emit `target` (field keys stayed `to` / `to_player` so existing `openActionForm` presets + adjacent-sector clicks Just Work). Fix 2 (autopilot — **blocker**): Grok kept emitting `{"destination": 42}` for `plot_course`, tripping the strict-arg validator; after 3 consecutive rejects the `TaskAgent` aborted the whole `profit_loop` with `state="error"` and the user saw crickets. Two-part fix in `src/tw2k/copilot/tools.py`: (a) `plot_course` param description rewritten to *"Target sector id (final destination)"* so the prompt itself stops seeding the wrong token; (b) new `_ARG_SYNONYMS` map + `ToolCall._normalize_synonyms()` (called at the top of `validate_against_catalog`) that rewrites `destination`/`dest`/`to`/`sector`/`sector_id`/`goal` → `target`, `planet`/`planet_idx` → `planet_id`, `player`/`opponent`/`enemy`/`other` → `target_id`, `ship`/`class`/`model` → `ship_class`, `quantity`/`amount`/`count`/`n` → `qty`, `text`/`msg`/`body` → `message`. Conservative rules: only rewrites when the canonical key exists on *that* tool's schema AND isn't already present — never overwrites explicit values, never invents params on tools that don't accept them. Fix 3 (nav UX): spectator `/` had a one-way trip to `/play` but no way back — added `🧑 Cockpit` anchor in `web/index.html` header controls with matching `.cockpit-link` styles in `web/style.css`, so the reverse navigation now works both directions. Regression sweep: existing H2 + H5 suites (57 tests) all green with the synonym normalizer in the path; live match re-run showed Grok's `profit_loop` task running 6+ iterations (`scan_port` / `scan` / `plot_course` / `warp` / `warp`) with zero validator rejects, P1 auto-moved from sector 1 → 188 hands-off. Also refreshed README + `docs/USER_GUIDE.md` + `docs/ROADMAP.md` with the full human-copilot story (Phase H series, `/play` cockpit, four-mode spectrum, voice + memory + what-if + tracing feature list) — the AI-vs-AI-focused docs hadn't mentioned the human path at all. | H5 as-shipped had two blockers that only showed up in live play; H5.5 fixes them and documents the full Phase H surface. |
