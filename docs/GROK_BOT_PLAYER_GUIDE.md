...
- `detail:"precise"` for **all 34 verbs** (S4): every handler precondition is mirrored, including the expensive ones — `attack` / `photon_missile` in FedSpace are reported illegal *before* the engine docks your alignment. Useful envelope extras: `buy_ship.params.ship_class.{choices, net_cost_by, trade_in, blocked_by}`, `buy_equip.params.item.{choices, unit_price_by}` + `qty.max_by[item]` (affordability × capacity), `build_citadel.params.next.{level, credits, colonists, colonists_have, days, pay_from}`, `deploy_genesis.params.{hops_from_stardock, min_hops}`, `land_planet.params.planet_id.contested` (defended hostile planets = combat on landing), `assign_colonists.params.{from.choices, to.choices, qty.max_by, qty.ship_free}`, `accept_alliance / break_alliance.params.alliance_id.choices`. `deploy_atomic` is never legal: the engine has no handler for it — use `deploy_mines` with `kind=atomic`.
