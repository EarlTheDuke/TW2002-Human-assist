...
def _check_victory(universe: Universe) -> None:
    ...
    if len(alive) == 1 and len(universe.players) > 1:
        ...
        universe.emit(
            EventKind.GAME_OVER,
            actor_id=alive[0].id,
            payload={"reason": "elimination", "net_worth": alive[0].net_worth},
    ...
    if universe.day > universe.config.max_days:
        winner = max(alive, key=lambda p: p.net_worth) if alive else None
        universe.finished = True
        if winner:
            ...
            universe.emit(
                EventKind.GAME_OVER,
                actor_id=winner.id,
                payload={"reason": "time_net_worth", "net_worth": winner.net_worth},
                summary=f"GAME OVER — time expired; {winner.name} wins on net worth ({winner.net_worth}cr)",
            )
        else:
